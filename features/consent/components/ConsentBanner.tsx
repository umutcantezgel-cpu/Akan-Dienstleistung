        ...
        <div className="p-6 md:p-8 flex-1 overflow-y-auto max-h-[70vh] custom-scrollbar">
          <div className="flex items-start gap-4 mb-4">
            ...
            <div>
              <h2
                id="consent-title"
                className="text-large font-bold text-text-primary font-display mb-2 flex items-center gap-3 flex-wrap"
              >
                IHR DATENSCHUTZ IST UNSER FUNDAMENT
                <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-green-500/10 text-green-700 text-[10px] font-bold tracking-[0.1em] uppercase ring-1 ring-inset ring-green-600/20 shadow-sm align-middle">
                  <Check className="w-3 h-3" /> 100% DSGVO
                </span>
              </h2>
              <p
                id="consent-desc"
              ...
              >
                Wir nutzen Cookies und ähnliche Technologien. Einige sind
                technisch absolut notwendig (Essenziell), während andere uns
                helfen, diese Website und Ihre Erfahrung zu verbessern. Sie
...
function ConsentItem({
  ...
  return (
    <div
      className={`p-4 rounded-xl border ${checked ? "border-primary/20 bg-primary/5" : "border-border bg-white"} transition-colors relative`}
    >
      <div className="flex items-start justify-between gap-4">
        <div className="pr-12">
          <label
            htmlFor={id}
            className="font-bold text-sm text-text-primary block mb-1 cursor-pointer"
          >
            {title}
          </label>
        ...
        <div className="absolute right-4 top-4">
          <button
            ...
            className={`w-11 h-6 rounded-full transition-colors relative focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-primary ${
