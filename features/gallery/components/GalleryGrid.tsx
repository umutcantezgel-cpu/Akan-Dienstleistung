...
export default function GalleryGrid({ images, categories }: GalleryGridProps) {
    const allCategories = categories || ['Alle', ...Array.from(new Set(images.map((img) => img.category)))];
    const [activeCategory, setActiveCategory] = useQueryState('filter', { defaultValue: 'Alle' });
    const [selectedImage, setSelectedImage] = useState<GalleryImage | null>(null);

    const filtered = activeCategory === 'Alle' ? images : images.filter((img) => img.category === activeCategory);
        ...
        <div className="relative">
            ...
            <div className="flex flex-wrap justify-center gap-4 mb-14 relative z-10">
                {allCategories.map((cat) => {
                    const count = cat === 'Alle' ? images.length : images.filter(img => img.category === cat).length;
                    ...
                    return (
                        <motion.button
                            ...
                            className={`relative px-6 py-3 rounded-xl text-tiny font-bold tracking-wide transition-all border flex items-center gap-3 overflow-hidden ${isActive
                        ...
                        >
                            ...
                            {/* Partikel-Dot-Count */}
                            <div className={`flex items-center justify-center min-w-[24px] h-[24px] rounded-full text-micro font-bold transition-colors ${isActive ? 'bg-white/20 text-white' : 'bg-primary/5 text-primary'
                                }`}>
                                {count}
                            </div>
                        </motion.button>
                    ...
                    <AnimatePresence mode="popLayout">
                        {filtered.map((img, i) => (
                            <motion.div
                                ...
                                initial={{ opacity: 0, scale: 0.8, filter: 'blur(10px)', x: (i % 3 === 0 ? -20 : i % 3 === 2 ? 20 : 0) }}
                                animate={{
                                    opacity: 1,
                                    scale: 1,
                                    filter: 'blur(0px)',
                                    ...
                                    <div className="aspect-[4/3] relative overflow-hidden border-b border-border/50">
                                        ...
                                        <div className="absolute inset-0 bg-primary/0 group-hover:bg-primary/20 transition-colors duration-500 flex items-center justify-center">
                                            <motion.div
                                                ...
                                                transition={springs.snappy}
                                            >
                                                <Maximize2 className="w-6 h-6 text-primary" strokeWidth={2} />
                                            </motion.div>
                                        </div>
                                    </div>
                                    <motion.div layoutId={`gallery-text-${img.src}`} className="p-8">
                                        <span className="text-mini font-bold text-primary uppercase tracking-[0.2em] mb-2 block">{img.category}</span>
                                        <h3 className="text-large font-bold text-text-primary font-display tracking-tight leading-snug">{img.title}</h3>
                                    </motion.div>
                                </div>
                            </motion.div>
                        ))}
                    ...
                    {selectedImage && (
                        ...
                        >
                            ...
                            >
                                ...
                                <motion.div layoutId={`gallery-text-${selectedImage.src}`} className="p-8 lg:p-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6 bg-surface">
                                    <div>
                                        <span className="text-mini font-bold text-primary uppercase tracking-[0.2em] mb-2 block">{selectedImage.category}</span>
                                        <h3 className="text-2xl lg:text-3xl font-bold text-text-primary font-display tracking-tight leading-tight">{selectedImage.title}</h3>
                                    </div>
