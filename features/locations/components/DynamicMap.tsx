...
const LeafletMap = dynamic(() => import('./LeafletMap'), {
    ...
    loading: () => (
        <div className="absolute inset-0 z-20 w-full h-full flex flex-col items-center justify-center bg-[#0a0a0c]/80 backdrop-blur-xl border border-white/5">
            ...
            <span className="text-white/50 font-mono text-xs tracking-[0.3em] uppercase">Satellitendaten werden geladen</span>
        </div>
    ),
});
...
export default function DynamicMap({ location, zoom = 11 }: DynamicMapProps) {
    ...
    return (
        ...
        >
            ...
            >
                <h4 className="font-bold text-text-primary text-xl font-display mb-1">{location.name}</h4>
                <p className="text-sm font-medium text-primary uppercase tracking-widest mb-3">
                    Einsatzgebiet
                </p>
                <div className="text-xs text-text-secondary flex justify-center gap-4">
                    <span>{location.entfernung === 'Hauptstandort' ? 'Zentrale' : `${location.entfernung} Anfahrt`}</span>
                    <span>•</span>
