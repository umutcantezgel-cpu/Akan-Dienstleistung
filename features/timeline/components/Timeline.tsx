...
export default function Timeline({ events, className = '' }: TimelineProps) {
    ...
    return (
        <div ref={containerRef} className={`relative py-10 ${className}`}>
            ...
            {/* Mobile lines */}
            <div className="absolute left-8 top-0 bottom-0 w-1 bg-border/50 -translate-x-1/2 rounded-full md:hidden" />
            <motion.div
                className="absolute left-8 top-0 w-1 bg-primary -translate-x-1/2 rounded-full md:hidden origin-top shadow-[0_0_15px_var(--color-primary)] z-0"
                style={{ height: lineHeight }}
            />
...
function TimelineItem({
    ...
    return (
        ...
        >
            ...
            <div className="absolute left-8 md:left-1/2 -translate-x-1/2 flex items-center justify-center w-8 h-8 z-20">
                {/* Core Dot */}
                <motion.div
                    initial={{ scale: 0 }}
            ...
            <motion.div
                ...
                animate={
                    isInView
                        ...
                        : { opacity: 0, x: slideDirection, scale: 0.6, rotateY: isLeft ? 15 : -15 }
                }
                transition={{ delay: 0.15, ...springs.bouncy }}
                className={`ml-16 md:ml-0 md:w-1/2 w-full ${isLeft ? 'md:pl-16 md:pr-4' : 'md:pr-16 md:pl-4'} perspective-1000`}
            >
                <div className="bg-surface rounded-2xl p-8 border border-border/60 shadow-elevated relative overflow-hidden group hover:border-primary/40 transition-colors duration-500">
